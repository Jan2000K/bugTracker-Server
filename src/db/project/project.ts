...
export default class Project {
    ...
    static load = async (arrayOfIDs: number[]): Promise<Project[]> => {
        ...
        if (queriedProjects.length === 0) {
            ...
            for (let i = 0; i < arrayOfIDs.length; i++) {
                ...
                let tempArr = queriedProjects.filter((project) => {
                ...
                let generatedProject = new Project(
    ...
    static getAllProjects = async (): Promise<Project[]> => {
        ...
        if (res.length === 0) {
            ...
            for (let i = 0; i < arrayOfIDs.length; i++) {
                //Create a temp array in which there is only 1 project
                let tempArr = projectsData.filter((project) => {
                    if (project.projectID === arrayOfIDs[i]) {
                        return true
                    }
                })
    ...
    calculateBugStats = () => {
        ...
        for (let i = 0; i < this.bugs.length; i++) {
            ...
            if (
                bugInstance.status === "Open" ||
                bugInstance.status === "Testing"
            ) {
