...
export async function getAllUsers(){
    return await new pgQuery<userNoPassword>('SELECT "userID", "username" FROM public."User";',[]).exec()
}

export async function updateUser(user:User){
  let hashPass = await User.hashPassword(user.password)
  await new pgQuery('UPDATE "public.User" SET "username" =$1, "password"=$2',[user.username, hashPass]).exec()
  return true
}
...
export async function insertUser(user:User){
  ...
  await new pgQuery('INSERT INTO public."User" ("username","password") VALUES ($1,$2);',[user.username,hashPass]).exec()
}
...
export async function getUserByName(name:string){
  return await new pgQuery<user>('SELECT * FROM public."User" WHERE "username"=$1',[name]).exec()
}