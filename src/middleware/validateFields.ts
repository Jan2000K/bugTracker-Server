...
export function validateProject(
    ...
    if (!Project.isValidProject(body)) {
        res.json({
            err: true,
            msg: "Posted parameters failed field validation",
        })
    } else {
        next()
    }
...
export function validateIDQueryParams(
    req: idArrayRequest,
    res: Response,
    next: NextFunction
) {
...
export function validatePatchIDs(
    ...
    if (!passedBugs) {
        res.json({
            err: false,
            message: "Bug IDs have cannot be 0 when updating bugs",
        })
    } else if (body.id === 0) {
...
export function validatePostIDs(
    req: Request,
    res: Response,
    next: NextFunction
) {
    ...
    if (!passedBugs) {
        res.json({
            err: false,
            message: "Bug IDs must be set to zero when posting a new project",
        })
    } else if (body.id !== 0) {
        res.json({
            err: true,
            message:
                "Project ID must be set to zero when posting a new project",
        })
    } else {
        next()
    }
...
export function validateBugUpdate(
    req: Request,
    res: Response,
    next: NextFunction
) {
...
export function validateBugPost(
    req: Request,
    res: Response,
    next: NextFunction
) {
...
export function validateLoginPost(
    ...
    if (body.username && body.password) {
        if (
            ...
            res.json({
                err: true,
                message: "User credentials must be a string",
            })
        }
    } else {
