...
import {
    ...
    validatePostIDs,
    validateProject,
} from "../middleware/validateFields"
...
projectRouter.get(
    ...
    checkSession,
    validateIDQueryParams,
    async (req: idArrayRequest, res, next) => {
        try {
...
projectRouter.patch("/updateName", checkSession, async (req, res, next) => {
    ...
    if (body.projectID && body.name) {
        ...
        res.json({
            err: true,
            message: "Missing required keys (name, projectID)",
        })
    }
})
    ...
    async (req, res, next) => {
        try {
            ...
            if (body.id === 0) {
                res.json({
                    err: true,
                    message: "Project ID cannot be 0 if its being updated!",
                })
            } else {
                const project = new Project(body.name, body.bugs)
                project.id = body.id
